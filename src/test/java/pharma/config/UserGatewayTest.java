...
class UserGatewayTest {
    ...
    void valid_email_password() throws Auth0Exception {
        ...
        try(MockedStatic<InputValidation> input_validation_mock=Mockito.mockStatic(InputValidation.class)) {
            ...
            UserGateway userGateway = new UserGateway(stytchClient);
            UserGatewayResponse response = userGateway.authenticate("user@example.com", "@5&17Vhm5QGp");
    ...
    void Invalid_email_password() throws Auth0Exception {
        ...
        try(MockedStatic<InputValidation> input_validation_mock=Mockito.mockStatic(InputValidation.class)) {
            ...
            UserGateway userGateway = new UserGateway(stytchClient);
            UserGatewayResponse response = userGateway.authenticate("pinco.pallino@example","asss");
