...
public class SuggestPriceHandler extends DialogHandler<FieldData> {
    ...
    public SuggestPriceHandler(String content, SuggestPriceDao suggestPriceDao, PriceSuggestion priceSuggestion, SimpleBooleanProperty s_boolean) {
        super(content, List.of(suggestPriceDao));
        this.priceSuggestion=priceSuggestion;
        this.suggestPriceDao=suggestPriceDao;
    ...
    protected <K> void initialize(Optional<PopulateChoice<K>> PopulateChoice, Optional<List<GenericJDBCDao>> optionalgenericJDBCDao, Optional<FieldData> optionalfieldData) {
        if(optionalgenericJDBCDao.isPresent()){
            SuggestPriceDao suggestPriceDao = (SuggestPriceDao) optionalgenericJDBCDao.get().stream().filter(value -> value instanceof SuggestPriceDao).findFirst().get();
            this.search_product = add_SearchComboBox(FieldData.FieldDataBuilder.getbuilder().setNome("Cerca Farmaco").build());
            this.search_product.setItems(FXCollections.observableArrayList(suggestPriceDao.findNotExistPrice()));
    ...
    protected boolean condition_event(FieldData type) throws Exception {
         boolean value=suggestPriceDao.insert(type);
