...
import com.auth0.json.mgmt.resourceserver.TokenEncryption;
...
public class ThreadServerManager implements Runnable {
    ...
    private final BlockingQueue<pharma.test2.ChatMsg> outgoingMessages = new LinkedBlockingQueue<>();
        ...
        readerExecutor.execute(() -> {
            ...
            while (isConnected.get()) {
                try {
                    ...
                    if (obj instanceof pharma.test2.ChatMsg msg) {
    ...
    private void startWriter() {
        writerExecutor.execute(() -> {
            ...
            while (isConnected.get()) {
                try {
                    pharma.test2.ChatMsg msg = outgoingMessages.take(); // Blocks until a message is available
    ...
    private void processIncomingMessage(pharma.test2.ChatMsg msg) throws ExpireToken {
    ...
    public void sendMessage(pharma.test2.ChatMsg msg) {
        if (isConnected.get()) {
