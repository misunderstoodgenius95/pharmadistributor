...
import pharma.Handler.Table.OrderTableView;
...
public class SellerOrderHandler extends DialogHandler<FieldData> {
    private static final Logger log = LoggerFactory.getLogger(SellerOrderHandler.class);
    ...
    public SellerOrderHandler(String content, SellerOrderDao s_dao, SellerOrderDetails s_details, SellerInvoiceDao s_invoice, SellerCreditNoteDao s_credit,SellerCreditNoteDetailDao s_credit_detail) {
    ...
    protected <K> void initialize(Optional<PopulateChoice<K>> PopulateChoice, Optional<List<GenericJDBCDao>> optionalgenericJDBCDao, Optional<FieldData> optionalfieldData) {
        group_choice = add_radios(List.of(new RadioOptions("order_only", "Ordine"),
                        ...
                        new RadioOptions("all", "Tutti Ordini")),
    ...
    public void execute_query(String id, TableView<FieldData> tableView) {
        ...
        switch (id) {
            case "all" -> {
                observableList.addAll(s_dao.findAll());
            }
            case "order_only" -> {
                if (!isValidInteger(textField_search.getText())) {
                    Utility.create_alert(Alert.AlertType.WARNING, "WARNING", "Inserire un numero intero");
                } else {
                    observableList.addAll(s_dao.findById(Integer.valueOf(textField_search.getText())));
                }
            }
            case "header_range" -> {
                observableList.addAll(s_dao.findByRangeBetweenAndRagioneSociale(Date.valueOf(range_start.getValue()), Date.valueOf(range_end.getValue()), textField_search.getText()));
            }
            default -> {
