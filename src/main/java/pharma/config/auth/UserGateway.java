...
public class UserGateway {
    private static final Logger logger = Logger.getLogger(UserGateway.class.getName());
    private StytchClient stytchClient;

    public UserGateway(StytchClient stytchClient) {
        this.stytchClient = stytchClient;
    }

    public UserGatewayResponse authenticate(String email, String password) {
        ...
        if(response.statusCode()==404){
            return new UserGatewayResponse(response.body(),401);
        }
      boolean value=validate_user(response.body());
        if(value){
            return new UserGatewayResponse(response.body(), response.statusCode());
        }else{
            return new UserGatewayResponse(response.body(),403);
        }
    }
    ...
    public UserGatewayResponse authenticate_jwt(String jwt) throws ExpireToken {
    ...
    public UserGatewayResponse register(String email, String password, String role, String first_name, String surname) {
        ...
        if(jsonObject.has("error_type")) {
            ...
            if (error_type.equals("duplicate_email")) {
                return new UserGatewayResponse("Unprocessable Entity", 429);
            }
            return new UserGatewayResponse(response_create.body(), response_create.statusCode());
            ...
            return new UserGatewayResponse(response_reset.body(), response_reset.statusCode());

        }
        return new UserGatewayResponse(response_create.body(), response_create.statusCode());

    }
    public UserGatewayResponse register_pharmacist(String email, String password, String role, String first_name, String surname, int pharmacy_id) {
    ...
    public UserGatewayResponse searchUsers() {
    ...
    public UserGatewayResponse user_revocate(String user_id, boolean status) {
    ...
    public UserGatewayResponse user_update_role(String user_id, String role) {
        if (!InputValidation.validate_stytch_user_id(user_id)) {
            throw new IllegalArgumentException("User id  is not valid");
        }
   ...
   public static Optional<Instant> recent_last_access(Session session_model){
    ...
    public UserGatewayResponse GettingUserInfoByUserId(String user_id){
