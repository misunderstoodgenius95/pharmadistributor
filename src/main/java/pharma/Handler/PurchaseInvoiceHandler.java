...
public class PurchaseInvoiceHandler extends  DialogHandler<FieldData> {
    ...
    public PurchaseInvoiceHandler(  PharmaDao pharmaDao,PurchaseInvoiceDao p_inv_dao,PurchaseOrderDao p_dao,SimpleBooleanProperty s_update) {
    ...
    public PurchaseInvoiceHandler(PurchaseInvoiceDao p_invoice_dao,PurchaseOrderDao p_dao) {
        super("Content");
        this.p_dao=p_dao;
    ...
    protected  <K>void  initialize(Optional<PopulateChoice<K>> optionalpopulateChoice, Optional<List<GenericJDBCDao>> optionalgenericJDBCDao, Optional<FieldData> optionalfieldData) {
       if(optionalgenericJDBCDao.isPresent()) {
         ...
         /*  setting_id_radio();*/
           ...
           combo_order_provider_id.getChoiceBox().setValue(FieldData.FieldDataBuilder.getbuilder().setOriginal_order_id("Seleziona Ordine Id Fornitore").build());

           combo_order_provider_id.setConvert(new InvoicexConvert(InvoicexConvert.Type.combo_id));

           listen_toggle();
           // Default first  radio item
           toggleGroup_order_choice.getToggles().getFirst().setSelected(true);
           ...
           getControlList().removeAll(combo_pharma,combo_order_provider_id, comboBox_order);
       }
    }
    ...
    private  void listen_order_add_row(){
        ChangeListener<Object> changeListener=((observable, oldValue, newValue) -> {
            if(newValue!=null) {
                ...
                System.out.println("pharma " + fieldData.getCasa_farmaceutica());
        ...
        combo_order_provider_id.getChoiceBox().getSelectionModel().selectedItemProperty().addListener(changeListener);
      ...
      combo_pharma.getChoiceBox().valueProperty().addListener((observable, oldValue, newValue) -> {
          if(newValue instanceof FieldData fieldData){
              if(table_id.getItems().isEmpty()) {
                  ...
                  comboBox_order.getChoiceBox().setItems(list_populate);
                  combo_order_provider_id.getChoiceBox().setItems(list_populate);
              }
        ...
        toggleGroup_order_choice.selectedToggleProperty().addListener((observable, oldValue, newValue) -> {
           if(newValue instanceof RadioButton radioButton){
               if(radioButton.getId().equals("order_id")){
                   ...
                   combo_order_provider_id.setVisible(false);
                   comboBox_order.getChoiceBox().setItems(list_populate);

               }else{
                   comboBox_order.setVisible(false);
                   combo_pharma.setVisible(true);
                   combo_order_provider_id.setVisible(true);
    ...
    private void  setting_id_radio(){
        ...
        List<String> value=new ArrayList<>(Arrays.asList("order_id","combo_id"));
        for(int i=0;i<value.size();i++){
            RadioButton radioButton=(RadioButton) toggleGroup_order_choice.getToggles().get(i);
            radioButton.setFont(new Font(14));
            radioButton.setId(value.get(i));
        }
